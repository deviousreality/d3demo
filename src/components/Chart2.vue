...
var resetButton = d3.select('.g-controls').append('a')
    .attr('class', 'g-reset')
    .text('Reset')
    .on('click', function() { this.reset(); this.transition(); });
...
export default {
  ...
  created() {
    allStates = this.chartData.races.map(function(race) {
  ...
  controlButton = control.append('div')
    ...
    .enter().append('button')
      ...
      .on('click', function(view) { this.change(view); this.transition(); });
    ...
    this.chartData.board.races.forEach(function(race) {
  ...
  methods: {
    update() {
      ...
      //
      nodesByKey = {};
      nodes.forEach(function(d) { nodesByKey[d.key = this.key(d)] = d; });
    ...
    updateLinks() {
      ...
      //     .attr('d', function(d) { var p = this.findOldParent(d.target); return this.diagonal({source: p, target: p}); })
      ...
      d3.transition(link.exit())
          .attr('d', function(d) { var p = this.findParent(d.target); return this.diagonal({source: p, target: p}); })
          .remove();

      d3.selectAll('.g-link')
          .sort(this.ascendingDepth);
    ...
    updateNodes() {
      ...
      var nodeExit = d3.transition(node.exit())
          ...
          .attr('transform', function(d) { var p = this.findParent(d); return 'translate(' + p.x + ',' + y(p.depth) + ')'; })
          .remove();

      var nodeEnter = node.enter().insert('g', '.g-scoreboard')
          .attr('class', function(d) { return 'g-node g-' + d.party; })
          .attr('transform', function(d) { var p = this.findOldParent(d); return 'translate(' + p.x + ',' + y(p.depth) + ')'; });

      d3.transition(node)
          .attr('class', function(d) { return 'g-node g-' + d.party + (d.depth === 1 ? ' g-first' : ''); })
          .attr('transform', function(d) { return 'translate(' + d.x + ',' + y(d.depth) + ')'; });

      this.updateLabels(nodeEnter, nodeExit);
      this.updateLeaves(nodeEnter, nodeExit);
    },
    ...
    updateLabels(nodeEnter, nodeExit) {
      ...
      node.select('.g-label-line1')
          .attr('y', function(d) { return d.children ? null : -this.radius(d.depth) - 3; })
          .text(function(d) {
      ...
      node.select('.g-label-line2')
          .attr('y', function(d) { return this.radius(d.depth) + 3; })
          .text(function(d) {
    ...
    updateLeaves(nodeEnter, nodeExit) {
      ...
      d3.transition(leaf)
          .attr('transform', function(d) { return 'scale(' + this.radius(d.depth) + ')scale(.02)'; });
    },
    ...
    reset() {
      controlButton.filter(function(d) { return d.outcome === d.state.enabledParty; }).each(this.change);
    },
    ...
    transition() {
      ...
      this.mouseout();
      d3.transition().duration(d3.event && d3.event.altKey ? 7500 : 750).each(this.update);
    ...
    click(d) {
      ...
      if (d !== root) {
        this.change({state: d.state, outcome: d.party});
        this.transition();
      }
    },

    mouseover(d) {
      this.activate(activeOutcome = d, true);
    ...
    mouseout() {
      ...
      if (activeOutcome) {
        this.activate(activeOutcome, false);
        activeOutcome = null;
      }
    },
    ...
    activate(d, active) {
      ...
      d.parent && this.activate(d.parent, active);
    },
